...
class ImageSubmissionIT extends FacadeIT {
    ...
    void postImageSubmissionWithPngShouldReturn200() {
        ...
        body.add("file", new org.springframework.mock.web.MockMultipartFile(
                "file", "photo.png", "image/png", "fake png content".getBytes()));

        ResponseEntity<ImageSubmissionResponse> response = restTemplate.exchange(
                "/image-submissions",
                HttpMethod.POST,
                new HttpEntity<>(body, new HttpHeaders() {{ setContentType(MediaType.MULTIPART_FORM_DATA); }}),
    ...
    void getShouldReturnAllSubmissions() {
        ...
        restTemplate.exchange("/image-submissions", HttpMethod.POST,
                new HttpEntity<>(body, new HttpHeaders() {{ setContentType(MediaType.MULTIPART_FORM_DATA); }}),
                ImageSubmissionResponse.class);
    ...
    void postThenGetShouldPersist() {
        ...
        ResponseEntity<ImageSubmissionResponse> postResponse = restTemplate.exchange(
                "/image-submissions",
                HttpMethod.POST,
                new HttpEntity<>(body, new HttpHeaders() {{ setContentType(MediaType.MULTIPART_FORM_DATA); }}),
                ImageSubmissionResponse.class);
        ...
        assertThat(imageSubmissionRepository.findById(id).get().getEmail()).isEqualTo("persist@test.com");
    }
}
